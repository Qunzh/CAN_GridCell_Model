...
$h(\theta_i)$ is computed based on the cosine difference between the orientation $\theta_i$ which neuron is selecctive and the orientation $\theta_{cue}$ which thalamus input to V1, modulated by firing rate amplitude of thalamus ($A$) and contrast $c$ of $\theta_{cue}$. The tuning of thalamic input $\epsilon$ modulate the degree of which $\theta_{cue}$ dominates thalamic input in general
